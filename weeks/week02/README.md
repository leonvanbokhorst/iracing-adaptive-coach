# Week 02 - Rudskogen Motorsenter - Season 01 2026

**Track**: [Rudskogen Motorsenter](../../tracks/track-rudskogen-motorsenter.md)  
**Car**: [Ray FF1600](../../cars/car-ray-ff1600.md)  
**Dates**: December 18 → December 25, 2025  
**Status**: COMPLETE ✅ (5 races, championship points banked!)

---

## The Story

There's a specific kind of doubt that comes with returning to a track after six months away. Master Lonn felt it the moment Rudskogen's downhill came rushing at him—faster than he remembered, meaner than his muscle memory wanted to admit.

**Day 1**: The rust was real but localized. 1:30.290, gap to July: 1.321s. But 73% of that loss lived in ONE corner—Turn 2, that technical downhill. Everywhere else? The track flowed back like an old conversation.

**Day 3**: The breakthrough. Not in the data first, but in his hands. The "50 sign" marker became his anchor. Trail brake through, third gear at the apex, second before the hill climbs back up. T2 clicked. 1:29.691. Sub-1:30. The rust shook loose.

**Day 4 Morning**: He went hunting for the edge. Found it at 1:28.572—**beating July's ghost by 0.397 seconds**. Early throttle, harder braking, using the tires like he meant it. Nine incidents testing limits. Worth every one.

**Day 4 Afternoon**: Tried to dial it back 5% for consistency. Everything exploded. Not from going too hard—from going too soft with too much steering. The tires slid. The snap felt like betrayal. Then the discovery: checking the Garage 61 overlay, his foot was resting on the throttle during braking. 10-20% overlap. Brake + throttle + steering = weight transfer chaos. The smoking gun.

**Day 4 Evening**: The rebuild. Clean the inputs first. "FOOT UP" drill before every brake zone. The car felt right again.

**Day 5 Morning**: First race. Quali P2, set a **new race PB: 1:28.508**—faster than practice. Setup breakthrough: 57.5% brake bias felt "way more stable," enabled harder braking, cleaner technique. Then two mistakes spiraled into a spin. DNF. But the data whispered something else: **505 out of 2,179 drivers globally**. Top 500 at Rudskogen after six months away.

**Day 5 Afternoon**: Overdrove in quali (P10), then fought back position by position. P3 secured on lap 7 (1:28.620). Then the brain said "stay close to leaders" when nobody was behind him. Impulse overrode strategy. Spin. P5.

**Day 6**: The gladiator race. Got punted at T1, someone went airborne over him, grass excursion, chaos everywhere. P3 by lap 1's end with heart pounding. Then—2-3 laps later—calm returned. "I could think strategic again." Recognized the position was secure, backed off 5%, finished P3. Strategic maturity executed perfectly without being told.

**Day 7 Morning (Dec 24)**: POLE POSITION. 1:28.955 in quali. Then threw it away in 60 seconds. T1 spin lap 1, T1 spin lap 2. Same corner, same cold tires, same mistake. P1 → P5. The pace was there (1:28.613 in the race, faster than quali), but cold tire discipline wasn't.

**Day 7 Afternoon (Dec 24)**: P2 quali. Progress: survived T1! Then immediately tried a passing move on lap 1. On cold tires. Went wide. P2 → P4. But after that? Seven clean laps, 0.377s consistency, fought back to P4 in a stronger field (SoF 1567). Against better drivers, held his ground. **iRating: 1494** (+256 from season start).

Week 02: From rusty (1:30.290) to championship-ready (1:28.5 pace, top 500 globally, strategic maturity). Not just faster—**smarter**. The only frontier left: surviving the first 90 seconds of races without the impulse winning.

---

## The Numbers

| Metric               | Start (Dec 18) | End (Dec 24)                      | Change      | Notes                             |
| -------------------- | -------------- | --------------------------------- | ----------- | --------------------------------- |
| **Best Lap**         | 1:30.290       | **1:28.508** (race)               | **-1.782s** | Beat July PB by 0.461s!           |
| **Consistency (σ)**  | 1.43s          | 0.336s (Dec 23), 0.377s (Dec 24)  | **-1.05s**  | Elite even in chaos               |
| **Gap to Target PB** | +1.321s        | **-0.461s** (BEAT IT!)            | **-1.782s** | Same-series PB demolished         |
| **S2 Focus Area**    | 31.81s (rusty) | 30.51s (locked)                   | **-1.30s**  | T2 breakthrough permanent         |
| **iRating**          | 1377           | 1494                              | **+117**    | Total +256 from season start      |
| **Global Ranking**   | Unknown        | **505 / 2,179 (Top 23.2%)**       | N/A         | Rudskogen Ray FF1600 (S1 + S4)    |
| **Brake Bias**       | 56%            | **57.5%** (breakthrough)          | **+1.5%**   | More stable, harder braking       |
| **Race Pace**        | N/A            | 1:28.5-1:30.0 (context-dependent) | N/A         | Attack vs survival pace confirmed |

**Week Stats:**

- Sessions: 10 total (5 practice, 5 races)
- Total laps: ~200+
- Clean laps: ~120+
- Incidents: ~70 (limit testing + learning)
- Podiums: 1x P3
- Pole Positions: 1x P1
- Race finishes: 3 complete (P3, P4, P5), 2 DNF (but with PBs!)
- Personal Bests: 2 (1:28.572 practice, 1:28.508 race)
- Global Ranking Achievement: Top 500 (505 / 2,179)
- iRating gained: +117 (1377 → 1494)

---

## Session Log

| Date                                                               | Time     | Type                 | Best Lap     | σ       | Result            | Key Takeaway                                            |
| ------------------------------------------------------------------ | -------- | -------------------- | ------------ | ------- | ----------------- | ------------------------------------------------------- |
| [Dec 18](2025-12-18-11-19-rudskogen-practice.md)                   | 11:19    | Practice             | 1:30.290     | 1.43s   | Baseline          | T2 rusty, rest remembered                               |
| [Dec 20](2025-12-20-15-22-rudskogen-practice.md)                   | 15:22    | Practice             | 1:29.691     | 0.70s   | Sub-1:30!         | T2 breakthrough: 50 sign brake marker                   |
| [Dec 21 AM](2025-12-21-11-10-rudskogen-practice.md)                | 11:10    | Practice             | **1:28.572** | 1.08s   | **NEW PB!**       | Beat July by 0.397s! Early throttle technique           |
| [Dec 21 PM](2025-12-21-14-18-rudskogen-practice.md)                | 14:18    | Practice             | 1:28.913     | 2.94s   | Paradox           | Throttle overlap discovered, snap oversteer             |
| [Dec 21 Eve](2025-12-21-16-26-rudskogen-practice.md)               | 16:26    | Practice             | 1:29.128     | 1.30s   | Rebuild           | Clean inputs restored, foot position fixed              |
| [Dec 22 AM](2025-12-22-10-46-rudskogen-race-first-race-week02.md)  | 10:46    | AI Race              | **1:28.508** | 3.82s   | DNF (P2→9)        | **RACE PB! + Setup breakthrough (57.5% BB) + Top 500**  |
| [Dec 22 PM](2025-12-22-14-13-rudskogen-race-second-race-week02.md) | 14:13    | AI Race              | 1:28.620     | 0.415s  | P5 (P10→P3)       | Impulse control lesson: P3 secured, unnecessary risk    |
| [Dec 23](2025-12-23-10-57-rudskogen-race.md)                       | 10:57    | Race                 | 1:30.020     | 0.336s  | **P3 🥉**         | **Strategic maturity: survival pace, position secured** |
| [Dec 24 AM](2025-12-24-05-24-rudskogen-race.md)                    | 05:24    | Race                 | 1:28.613     | 1.89s   | **P5** (P1→P5)    | **POLE! But threw it away: T1 spins lap 1 & 2**         |
| [Dec 24 PM](2025-12-24-13-57-rudskogen-race.md)                    | 13:57    | Race                 | 1:29.108     | 0.377s  | **P4** (P2→P4)    | Survived T1! But passing attempt lap 1 on cold tires    |
| [**Dec 27**](2025-12-27-post-season-race-retrospective.md)         | ~Unknown | **Post-Season Race** | 1:29.391     | Unknown | **P9** (P5→P2→P9) | **THE FEAR LOOP: Discovered fear of being overtaken**   |

---

## Breakthroughs 🎯

**1. T2 Downhill Braking Technique (Dec 20)**  
Found the "50 sign" brake marker, complete trail braking sequence, gear timing (3rd apex → 2nd before hill), and second apex vision. S2 average dropped from 31.81s to 31.15s (-0.66s), consistency improved 2.3x (0.75s → 0.32s σ). This single corner breakthrough unlocked the entire lap.

**2. NEW Personal Best - Beat July PB (Dec 21 AM)**  
1:28.572 practice lap, beating July same-series PB (1:28.969) by 0.397s. Key factors: early throttle application (+0.193g more acceleration = 26% increase!), harder braking for less time (5.4% reduction but faster), using +0.323g more lateral grip (14% increase), better steering efficiency (7.50 vs 6.52 g/deg). Gap to theoretical optimal only 0.123s (two corners).

**3. RACE PB + Setup Breakthrough (Dec 22 AM)**  
1:28.508 in first AI race (0.064s faster than practice PB!). Critical discovery: 57.5% brake bias (vs 56.5% practice) felt "way more stable" and enabled: +24.5% harder braking pressure, +6.5% more lateral G usage, +20% better steering efficiency. Master Lonn's instinct about needing ~57% BB was CORRECT—just needed clean inputs first to use it properly.

**4. Top 500 Globally (Dec 22)**  
Ranked 505 out of 2,179 drivers (top 23.2%) at Rudskogen Ray FF1600 across S1 2026 + S4 2025 combined. Faster than 1,674 drivers who raced this track/car over an entire year. In 5 days from rusty to top quarter globally.

**5. Throttle Overlap Discovery (Dec 21 PM)**  
By checking Garage 61 throttle/brake overlay visualization, Master Lonn discovered he was unconsciously resting his foot on the throttle during braking (10-20% overlap). This caused brake + throttle + steering simultaneously = weight transfer chaos = snap oversteer feeling. Elite self-diagnosis using visual data. Applied "FOOT UP" drill to fix it.

**6. Strategic Maturity - Position Secured Protocol (Dec 23)**  
In gladiator race (got punted, someone flew over him), Master Lonn survived chaos, recognized "P3 secured, 3s gap, leaders faster" → backed off 5% → prioritized finish → P3 podium. Applied the Dec 22 PM lesson (unnecessary risk when P3 secured) PERFECTLY without being told. Can now CHOOSE when to push vs hold = racing, not just driving.

**7. POLE POSITION (Dec 24 AM)**  
First pole position of the season: 1:28.955 quali lap. Front row, P1 on the grid. Proved qualifying mastery is developing (P1, then P2 back-to-back races). The challenge: executing on cold tires once the lights go out.

---

## Challenges 🚧

**1. Race Start Cold Tire Discipline (Dec 24 AM & PM)**  
Two races in a row, threw away podiums in the first 30-90 seconds. Dec 24 AM: Pole → P5 (spun T1 twice on cold tires, laps 1 & 2). Dec 24 PM: P2 → P4 (survived T1, then immediately tried passing move on cold tires, went wide). Both times: knew it was wrong, impulse won anyway. The gap isn't speed (99.6-99.8% of perfect when clean), isn't racecraft (mid-race execution elite), isn't mental resilience (post-mistake recovery perfect). It's 30-90 seconds of impulse control at race starts. Cold Tire Contract v2.0 needed: "NO passing attempts lap 1-2, period."

**2. The Commitment Paradox (Dec 21 PM)**  
Tried to "back off 5% for consistency" after setting PB, but backed off in the WRONG places: softer braking (-11%), less acceleration (-25%), less lateral G (-9%), but 40% MORE steering. Result: tires sliding instead of gripping. The lesson: backing off brake/throttle pressure while adding steering = sliding. 95% pace means 95% brake PRESSURE (still hard), 95% steering INPUT (smoother), not backing off commitment while fighting the car.

**3. Throttle Overlap Hardware Issue (Dec 21 PM)**  
Recent pedal changes (longer throttle travel, softer spring) → unconscious foot resting at 10-20% throttle during braking. Three conflicting inputs (brake tries to shift weight forward, throttle tries to shift weight backward, steering tries to turn) = confused weight transfer = snap oversteer. Required conscious "FOOT UP" drill to eliminate.

**4. Impulse Control When Position Secured (Dec 22 PM)**  
Jefferson Dec 13 pattern repeating: When goal is ACTIVE (catch P3), strategic/smart/fast. When goal is ACHIEVED (P3 secured, gap safe, leaders faster), ADHD brain creates ARTIFICIAL PRESSURE ("stay close to leaders!") → impulse overrides strategy → unnecessary risk → spin. KNEW it was unnecessary IN THE MOMENT but did it anyway = classic ADHD impulse control challenge. Dec 23 showed this CAN be overcome with conscious execution of Position Secured Protocol.

**5. Overdriving in Qualifying (Dec 22 PM, Dec 24 AM quali)**  
Same Jefferson Week 01 pattern: trying too hard in qualifying = slower. Dec 22 PM qualified P10 by overdriving (1:30.153), then ran 1:28.620 in race. Dec 24 AM quali (1:28.955) went faster in race (1:28.613). Need pre-quali calm routine: "Calm mind first, fast lap second." Don't force speed, let it come.

---

## What We Learned

**Technical:**

- **Early throttle application is REAL**: +0.193g more acceleration (26% increase!) by getting on power "as soon as turned in before apex." This is the breakthrough technique that transferred from Jefferson.
- **Brake bias optimization**: 57.5% BB (vs 56.5%) enables harder braking with better car control for Master Lonn's driving style (trail braking, modulation, early throttle).
- **Specific brake markers > feel**: "50 sign" marker = visual, repeatable, no guessing. Perfect for ADHD brain. Consistency followed understanding.
- **Throttle overlap detection**: Visual data (Garage 61 overlay) caught what numeric telemetry missed. Always check ALL inputs, not just brake/steering.
- **99.6-99.8% of perfect is achievable**: When clean, gap to theoretical optimal is 0.155-0.312s (ONE corner). The pace is championship-level.

**Mental:**

- **Mental reset speed improving**: Chaos → strategic calm now takes 2-3 laps (was 5-6 at Jefferson Week 01). ADHD brain learning to reboot faster under pressure.
- **Position Secured Protocol WORKS**: Recognizing "goal achieved, position safe, back off 5%, finish clean" = elite race IQ. Applied perfectly Dec 23 without being told.
- **Race start impulse control is THE frontier**: Speed confirmed (1:28.5), setup optimized (57.5% BB), technique locked (early throttle, S2 mastery), mid-race execution elite. Only challenge: first 30-90 seconds of races (cold tires + adrenaline + defending/attacking = impulse overrides strategy).
- **ADHD negative bias is STRONG**: Master Lonn said "I screwed up, I'm sorry" after setting RACE PB and ranking top 500 globally. Hyperfocus on mistakes (spin, incidents) completely blinds to massive achievements (PB, ranking, setup breakthrough). Need post-race ritual: "What went RIGHT first."
- **Self-awareness vs execution gap**: KNOWS cold tire discipline needed, KNOWS not to attack on lap 1-2, but impulse wins when opportunity appears. The pattern: brain finds justification ("he jumpstarted," "there's an opening," "defend pole") to override mental contract.

**Strategic:**

- **Process over results**: Dec 21 evening session wasn't about lap times—it was about fixing throttle overlap. That's elite driver behavior: diagnose root cause with data, apply fix, verify.
- **Limit testing has value**: Dec 21 AM racked up 9 incidents testing edges, but found 0.4s by discovering the LIMIT. Now knows where 100% is, can dial back to 95% intelligently.
- **Quality over quantity**: 20 focused laps with specific mission > 50 laps exploring aimlessly. Focused practice (T2 work Dec 20) delivered immediate breakthrough.
- **Setup supports technique, doesn't replace it**: 57.5% BB only works because technique is solid underneath. Needed clean inputs FIRST (throttle overlap fixed), THEN higher BB unlocked performance.
- **Strategic racing > fast driving**: P3 survival pace (1:30.020) earned same points as P3 attack pace would have. Mission was FINISH, not BE FASTEST. Smart > heroic.
- **Stronger fields = better performance**: SoF 1567 (Dec 24 PM) produced clean, consistent racing (0.377s σ) and P4 against better drivers. Competition creates focus, not chaos.

---

## Next Week Preview

**Track**: Winton Motor Raceway - National  
**Challenge**: New track, shorter lap (~1:30-1:35 range), technical, tight  
**Goal**: Apply Week 02 lessons to new environment, establish baseline, execute Cold Tire Contract v2.0 in races  
**Strategy**: Use Week 02 pattern that worked:

1. First session: Establish baseline, identify weak sector
2. Second session: Find breakthrough technique for weak sector
3. Practice sessions: Build consistency at 95% of limit, don't overdrive
4. Race quali: Pre-quali calm routine ("Calm mind first, fast lap second"), 95% pace over forcing 100%
5. **Race starts: COLD TIRE CONTRACT v2.0** - NO passing attempts lap 1-2, survive T1, hold line, wait until lap 3
6. Mid-race: Execute Position Secured Protocol, Incident = Survival Lap
7. Let PBs come naturally when flow appears, don't chase them

Week 02 proved: The speed is REAL (top 500 globally), the technique is PERMANENT (S2 breakthrough stuck under pressure), the setup is OPTIMIZED (57.5% BB confirmed), the strategic maturity is DEVELOPING (Position Secured Protocol works). The only gap: 30-90 seconds of race start impulse control. That's the mission for Week 03.

---

## Post-Season Epilogue: The Fear Loop Discovery (Dec 27)

After Week 02 officially ended, Master Lonn went back to Rudskogen for "one more race."

What happened next became one of the most important lessons of the season.

**The Race:**

- Qualified P5 (1:29.391, close to PB)
- Clean start → P4 → P2 (excellent racecraft)
- Then: Spun at S3 left-hander, Lap 3 → P7
- **Same corner, same spin, Lap 4** → Dead last
- Finished P9/12, iRating -39, SR +0.06

**The Discovery:**

Not competitive urge. Not wanting to pass. **Fear of being overtaken.**

When defending P2, Master Lonn's brain went into fear mode:

- "Don't lose this position. Don't mess up."
- Tension → tight grip → overdriving → spin
- Lap 4: "Don't spin here again" → spun again (Pink Elephant Problem)

**The Breakthrough Quote:**

> "It's not my pace, it's not my prep or my coach or my car or the others... most of the times it's me. And that can be fixed."

This is **elite self-awareness** most drivers never achieve.

**The Lesson:**

Fear makes you tight. Flow makes you fast.

Master Lonn discovered he needs to practice **"meebewegen"** (Dutch: going with the flow) under pressure, so it becomes automatic. When the race is on, the thinking brain shuts down. The **practiced brain** takes over.

Right now, the practiced brain under pressure is: "GRIP HARDER, FIGHT MORE, DON'T LOSE."

That needs to change.

**The Fix:**

Created the **"Let Them By" drill**: Practice being overtaken until it doesn't trigger fear. Let drivers pass, follow them, study their line, don't re-pass. Desensitization training.

**Why This Matters:**

The pace is there (1:29.391 near PB).  
The skill is there (P2 racecraft was clean).  
The respect is there (SR +0.06 despite spins).

What's missing: **Mental resilience under position-defense pressure.**

And that can be trained. Just like braking points. Just like racing lines.

**Guidebook Update:** This discovery is now documented in **Chapter 12 (Mental Game), Part 4: The Enemy - Fear and Tension**.

→ **See:** [Post-Season Race Retrospective](2025-12-27-post-season-race-retrospective.md) for the full honest story.

---

_Week 02 Complete: From rusty to top 500 globally in 7 days. From 1:30.290 to 1:28.508. From reactive to strategic. From fast driver to racer. The pace is there. The setup is dialed. The mid-race execution is elite. Now: survive the lights-out moment, train the mental game, and podiums are waiting. 🏎️💨_

_**Post-Script:** One more race taught the most important lesson of all—it's not always about going faster. Sometimes it's about getting out of your own way._ 🥋
