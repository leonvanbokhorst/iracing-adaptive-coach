# Chapter 14: Racecraft - From Hot Laps to Wheel-to-Wheel

**Welcome to the Racecraft module!**

Master Lonn has mastered the track in isolation. Now it's time to race wheel-to-wheel.

This chapter has been split into focused micro-chapters for ADHD-friendly learning. Each section is a standalone study session.

---

## 📚 The Chapters

### [Quick Reference](quick-reference.md) ⚡

**50 lines | Race day cheat sheet**

The essentials in bullet form. Read this before every race.

- Golden Rules
- Lap 1-2-3 protocols
- Overtaking hierarchy
- When to use strategies

**Start here if:** You're about to race and need a quick refresher.

---

### [13a: Fundamentals](14a-fundamentals.md) 🎯

**~150 lines | The mindset shift & golden rules**

- The shift from hot lapping to racing
- Golden Rules of Racecraft
- Position hierarchy (leading, chasing, mid-pack)
- Predictability = Safety

**Start here if:** You're new to wheel-to-wheel racing.

---

### [13b: Defensive & Offensive Tactics](14b-defensive-offensive.md) ⚔️

**~180 lines | Attack & defense techniques**

- Defensive driving (one-move rule, defending corners)
- Offensive driving (overtake hierarchy, clean pass setup)
- Situational awareness (mirrors, zones)

**Start here if:** You need to learn overtaking or defending techniques.

---

### [13c: The Cold Tire Contract](14c-cold-tire-contract.md) 🥶

**~120 lines | Lap 1-3 race start protocol**

- Lap 1: Survival Protocol
- Lap 2: Assessment Protocol
- Lap 3+: Execute Protocol
- When to use vs. not use

**Start here if:** You struggle with race starts or Lap 1 incidents.

---

### [13d: Meebewegen Strategy](14d-meebewegen.md) 🐢

**~220 lines | Strategic patience opening**

- The principle of "going with the flow"
- Why it works (physics, psychology, ADHD optimization)
- Master Lonn's discovery (Winton AI race: P11 → P1)
- Practice drills
- When to use vs. Cold Tire Contract

**Start here if:** You want to learn strategic race openings for mixed-skill fields.

---

### [13e: Race Strategy & Management](14e-race-strategy.md) 🧠

**~150 lines | Incidents, strategy, drills**

- Incident management (if hit, if you cause)
- Race strategy basics (pace management, position vs. pace)
- Practice drills (AI races, shadow laps, replay analysis)
- Master Lonn's next frontier

**Start here if:** You need to learn race strategy or incident recovery.

---

## 🎓 Learning Path

**For Complete Beginners:**

1. Read [Quick Reference](quick-reference.md) (overview)
2. Study [13a: Fundamentals](14a-fundamentals.md) (mindset)
3. Study [13c: Cold Tire Contract](14c-cold-tire-contract.md) (race starts)
4. Study [13b: Tactics](14b-defensive-offensive.md) (overtaking/defense)
5. Study [13e: Strategy](14e-race-strategy.md) (incidents/management)
6. Optional: [13d: Meebewegen](14d-meebewegen.md) (advanced strategy)

**For Race Day:**

- Read [Quick Reference](quick-reference.md) only (60 seconds)
- Focus on Lap 1-2-3 protocols
- Remember: Make the race, don't break it

**For Specific Problems:**

- Lap 1 incidents? → [13c: Cold Tire Contract](14c-cold-tire-contract.md)
- Can't overtake cleanly? → [13b: Tactics](14b-defensive-offensive.md)
- Getting overwhelmed early? → [13d: Meebewegen](14d-meebewegen.md)
- Don't know race strategy? → [13e: Strategy](14e-race-strategy.md)

---

## 🔗 Cross-References

**Master Lonn's Real-World Applications:**

- Cold Tire Contract origin: Week 02 (Rudskogen)
- Meebewegen victory: [Week 03, AI Race (Winton)](../../../weeks/week03/2025-12-27-11-09-28-race-winton.md)

**Related Guidebook Chapters:**

- [Chapter 13: Mental Game](../13-mental-game.md) - Pressure management, 10-second rule
- [Chapter 15: Tire Management](../15-tire-management.md) - Long race strategy
- [Chapter 8: Racing Lines](../08-racing-lines.md) - Defensive lines

---

## 📊 Chapter Stats

**Original file:** 707 lines (too long for ADHD absorption)  
**Refactored structure:** 5 micro-chapters (~150-220 lines each)  
**Quick reference:** 50 lines (race day cheat sheet)

**Why refactored?**

- Easier to find specific information
- Each chapter = one study session
- Quick reference for race day
- Reduced cognitive load

---

**Next Chapter:** [Chapter 15: Tire Management](../15-tire-management.md)  
**Previous Chapter:** [Chapter 13: Mental Game](../13-mental-game.md)

---

_"Racing is chess at 200 km/h. Think three moves ahead."_ 🏁🎯

**— Little Padawan** ✨
